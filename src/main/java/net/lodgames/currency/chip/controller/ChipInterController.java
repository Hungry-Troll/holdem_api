...
public class ChipInterController {
    ...
    @PutMapping("/user/{userId}/chip/cheat/{amount}")
    public ResponseEntity<?> chipCheat(@PathVariable("userId") Long userId, @PathVariable("amount") Long amount){
        chipService.chipCheat(userId, amount);
        return ResponseEntity.ok().build();
    }

    @PostMapping("/user/{userId}/chip/deposit")
    ...
    @PostMapping("/user/{userId}/chip/withdraw")
