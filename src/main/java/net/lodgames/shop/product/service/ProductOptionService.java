...
public class ProductOptionService {
    ...
    public void handleOrderProductOptions(Long userId, Long productId, Long depositRecordId, Long orderId) {
        // 상품 옵션 정보 취득
        List<ProductOption> productOptions = productOptionRepository.findAllByProductId(productId);
        // 복수의 상품 옵션 처리
        for (ProductOption productOption : productOptions) {
            // 상품 옵션 처리
            handleOrderProductOptions(productOption, userId, depositRecordId, orderId);
        }
    }

    // 주문한 상품 옵션을 처리 한다.
    public void handleOrderProductOptions(ProductOption productOption, Long userId, Long depositRecordId, Long orderId) {
        // 상품 옵션 타입
        ...
        switch (productOptionType) { // 상품 옵션 타입
            case DIAMOND -> { // 다이아몬드
                diamondService.addDiamondByOrder(userId, productOption.getQuantity(), idempotentKey);
